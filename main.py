"""
IELTS Bot — Essay & Speaking Scorer v2.8.1
──────────────────────────────────────────
...
MODEL_NAME = os.getenv("OPENAI_MODEL", "gpt-3.5-turbo")        # fallback
...
async def _reply_with_score(msg: Message, band: int, tips: list[str]) -> None:
    ...
    if credits is not None and credits <= 5:
        await msg.answer("⚠️ Only "
                         f"{credits} credit(s) left. Use /plans to top-up.")
...
async def cmd_write(msg: Message):
    ...
    try:
        ...
        async with get_pool() as pool:
            ...
            await pool.execute(
                "UPDATE users SET credits_left = GREATEST(credits_left - 1, 0) "
                "WHERE id=$1", msg.from_user.id,
            )
...
async def payment_success(msg: Message):
    # payload looks like  "plan:starter:25"
